...
int main(int argc, char *argv[])
{
    ...
    // --- lokalne bufory dla synchronizacji
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        // --- dwufazowe obliczenia bez false sharing
        #pragma omp parallel
        {
            // Faza 1: obliczanie sił - każdy wątek zbiera w rejestrach
            #pragma omp for schedule(static)
            for (int i = startIdx; i < endIdx; i++) {

                double fx = 0.0, fy = 0.0, fz = 0.0;

                // Wczytaj pozycję raz do rejestrów
                double xi = X[i];
                double yi = Y[i];
                double zi = Z[i];

                // Hot loop - oblicz siły
                for (int j = 0; j < N; j++) {
                    if (i == j) continue;
                ...
                // Aktualizuj prędkości (każdy wątek pisze do RÓŻNYCH i)
                VX[i] += fx * dt;
                VY[i] += fy * dt;
                VZ[i] += fz * dt;
            }

            // Implicit barrier - wszystkie prędkości gotowe
            
            // Faza 2: aktualizacja pozycji
